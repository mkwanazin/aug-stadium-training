...
- **BR5 (assumption — confirm before building)** — The finance CSV export carries account numbers **unmasked**. Inherited BR-11 (project.md) obfuscates sensitive fields "wherever a transaction is shown to a reviewer"; this epic reads the export as data handed to a downstream team to act on, not an on-screen reviewer presentation, so masking is assumed not to apply to it. Given the POPIA domain is active on this project, confirm this reading explicitly before shipping — if it's wrong, the export needs its own masking/reveal-audit story.
...
3. **Export approved transactions for finance** — the Approver chooses an export scope (everything approved in the period, a single file, or approved-and-not-yet-exported), sees how many approved files are in scope, and clicks `Export approved transactions`. Where the scope is non-empty, a CSV download is produced (columns per BR3) and a confirmation toast names how many files were covered. Where the scope is empty, no file is produced and the outcome names the empty scope (R5/BR1).
...
- **Export "not yet exported" scope has no backing field.** The aside's third scope option, `Approved and not yet exported`, implies a persisted per-transaction export flag or an export log that `TransactionRead` does not carry and the spec does not expose. Confirm whether this is added server-side (new field/endpoint) or approximated client-side (e.g. a local record of previously-exported transaction IDs, which would not survive across devices or sessions) before building that scope option.
