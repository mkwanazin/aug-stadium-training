...
- **RoleRead** — `Id`, `Name`. The values the API actually returns, verified live against `GET /v1/auth/userinfo` on 2026-08-26, are **`File Importer`** and **`Approver`** — the design and `documentation/requirements-application.md` write the first as `Importer`, which the API never returns. Match the verified table in project.md §Roles & Permissions, never the prose. The auth API's own example is `Viewer`, a role this project grants nothing to.
...
5. **BR5** — An Importer's role-gated nav/actions surface: may run session, upload, review and cancel flows (cancel gated by BR-15 in a later epic); has no access to Users, Roles, File Locations or Bulk File Settings. This epic uses the permission facts to decide what the shared nav shows an Importer — the Upload/Review/Cancel screens themselves are later epics' scope. Per the 2026-08-26 user decision (project.md §Roles & Permissions), an Importer also **sees** the `File settings` destination: it is gated on a view permission both roles hold, while administering file settings stays Approver-only. *(§6.5, Importer)*
...
- **"Request the Approver role" has no defined destination** (an open Uncertainty in the design digest, carried here because R12/UI-15's request-access path is this epic's to build) — BUILD will need a placeholder action (e.g., a mailto/notification stub) until the real workflow is specified. *Resolved in BUILD:* the destination is an administrator email address held in configuration — `NEXT_PUBLIC_ACCESS_REQUEST_EMAIL` (`web/.env.example`, `web/.env.local`), correctly browser-visible because it is a mailto destination rather than a backend origin. Taking the action opens the person's mail app pre-addressed and pre-written and confirms on screen, naming that address. No API endpoint carries the request — the Authentication API documents none.
