...
| `Permission`, `hasPermission`, `holdsAnyRole`, `rolesGranting`, `roleNamesOf`, `ROLE_IMPORTER`, `ROLE_APPROVER` | `web/src/lib/auth/permissions.ts` | The single grant table: does an account holding these roles hold this permission, and which roles grant it. Extend the `Permission` union + grant table per epic; never re-derive the check |
